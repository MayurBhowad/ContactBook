...
export class ContactsService {
  ...
  InData = JSON.parse(this.outData);

  constructor() {
    // external JOSN
    Object.entries(this.InData).map((e: any) => {
      if (e[0] < this.InData.length) {
        this.newContact.push(e[1]);
      }
    });
  }

  fetchContacts(): Observable<Contact[]> {
    return of(this.contacts).pipe(delay(4000));
  }

  addContact(newContact: Contact): Observable<Contact> {
    this.contacts.unshift(newContact);
    return of(newContact).pipe(delay(4000));
  }

  deleteContact(contact: Contact): Observable<any> {
    const index = this.contacts.findIndex((c) => c.name === contact.name);
    this.contacts.splice(index, 1);
    return of({ success: true }).pipe(delay(4000));
  }
}
